...
- Duplicate `Tool` interfaces in `core` and `@app-agent/tools`
...
2. Migrate to `@app-agent/tools` as canonical system
3. Merge both into new package

## Decision Outcome

**Chosen option**: `@app-agent/tools` is canonical.

- `AgentTool` base contract lives in `@app-agent/entities`
...
- Core imports `ToolRegistry` from `@app-agent/tools`
