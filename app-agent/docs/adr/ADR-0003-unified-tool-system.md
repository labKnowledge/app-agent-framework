...
- Duplicate `Tool` interfaces in `core` and `@gakwaya/app-agent-tools`
...
2. Migrate to `@gakwaya/app-agent-tools` as canonical system
3. Merge both into new package

## Decision Outcome

**Chosen option**: `@gakwaya/app-agent-tools` is canonical.

- `AgentTool` base contract lives in `@gakwaya/app-agent-entities`
...
- Core imports `ToolRegistry` from `@gakwaya/app-agent-tools`
