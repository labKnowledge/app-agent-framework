...
export class ToolRegistry extends EventEmitter {
   ...
   */
  async executeByName(name: string, params: unknown, context: ToolContext): Promise<string> {
   ...
   * Execute a tool
   */
  async executeTool(toolId: string, params: unknown, context: ToolContext): Promise<ToolResult> {
    const tool = this.tools.get(toolId);
    if (!tool) {
    ...
    try {
      ...
      if (this.config.enableCaching && tool.options?.caching?.enabled) {
        this.setCachedResult(toolId, params, toolResult);
      }
  ...
  private calculateRelevance(tool: Tool, query: ToolDiscoveryQuery): number {
    ...
    // Tags matching
    if (query.tags) {
      const tagMatches = query.tags.filter((tag) => tool.metadata.tags.includes(tag)).length;
  ...
  private getCachedResult(toolId: string, params: unknown): ToolResult | null {
  ...
  private setCachedResult(toolId: string, params: unknown, result: ToolResult): void {
    const cacheKey = this.getCacheKey(toolId, params);
  ...
  private updateMetrics(toolId: string, success: boolean, duration: number, cached: boolean): void {
    if (!this.config.enableMetrics) {
      return;
    }
  ...
  private createContext(_execution: ToolExecutionRequest): ToolContext {
    return {
      appState: {},
      execution: {
